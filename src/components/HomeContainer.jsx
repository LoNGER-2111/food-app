...
const HomeContainer = () => {
  return (
    <section className="grid w-full grid-cols-1 gap-2 md:grid-cols-2" id="home">
      <div className="flex flex-1 flex-col items-center justify-center gap-6 py-2 text-center md:items-start md:text-left lg:justify-start">
      ...
      <div className="relative flex flex-1 items-center py-2">
        ...
        <div className="absolute top-0 left-0 flex h-full w-full flex-wrap items-center justify-center gap-4 py-4 md:max-lg:top-[50%] md:max-lg:h-[90%] md:max-lg:translate-y-[-50%] lg:px-32 lg:max-[1439px]:px-12">
          {heroData &&
            heroData.map((data) => (
              <div
                key={data.id}
                className="flex flex-col items-center justify-center rounded-3xl bg-cardOverlay p-4 drop-shadow-lg backdrop-blur-md md:max-lg:w-[45%] lg:max-[1119px]:w-[47%] max-ssm:w-32 lg:w-190"
              >
                <img
                  src={data.imageSrc}
                  ...
                  alt={data.name}
                />
                <p className="mt-2 text-base font-semibold text-textColor md:max-lg:text-[12px] lg:mt-4 lg:text-xl lg:max-[1119px]:text-[14px] max-ssm:text-[10px]">
                  {data.name}
                </p>
                <p className="my-1 text-[12px] font-semibold text-lighttextGray md:max-lg:text-[10px] max-ssm:text-[8px] lg:my-3 lg:text-sm">
                  {data.desc}
                </p>
